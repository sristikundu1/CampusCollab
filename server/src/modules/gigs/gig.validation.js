...
const budget = z
  ...
  .superRefine((value, context) => {
    if (
      ...
      context.addIssue({
        code: "custom",
        message: "Unpaid gigs cannot include monetary values",
      });
    if (
      value.type !== "UNPAID" &&
      (!value.currency || value.minMinor === undefined)
    )
      context.addIssue({
        code: "custom",
        message: "Paid gigs require currency and an amount",
      });
    if (
      value.type === "RANGE" &&
      ...
      context.addIssue({
        code: "custom",
        path: ["maxMinor"],
        message: "Range maximum must be at least the minimum",
      });
  });
const gigFields = {
...
const updateBody = z
  .object(
    Object.fromEntries(
      Object.entries(gigFields).map(([key, schema]) => [
        key,
        schema.optional(),
      ]),
    ),
...
export const listGigsRequest = z.object({
  params: empty,
  query: listQuery,
  body: noBody,
});
export const listMineRequest = z.object({
  params: empty,
  query: mineQuery,
  body: noBody,
});
export const gigRequest = z.object({
  params: gigParams,
  query: empty,
  body: noBody,
});
export const updateGigRequest = z.object({
  params: gigParams,
  query: empty,
  body: updateBody,
});
export const transitionGigRequest = z.object({
  params: gigParams,
  query: empty,
  body: commandBody.optional().default({}),
});
export const bookmarkListRequest = z.object({
  params: empty,
  query: z
    .object({
      cursor: z.string().max(1024).optional(),
