...
const budgetSchema = new Schema(
  {
    type: { type: String, enum: ["FIXED", "RANGE", "UNPAID"], required: true },
    minMinor: intField({ required: false, min: 0 }),
    maxMinor: intField({ required: false, min: 0 }),
    currency: { type: String, uppercase: true, minlength: 3, maxlength: 3 },
  },
  { _id: false, strict: "throw" },
);

const gigSchema = addVersion(
  new Schema(
    {
      ownerId: { type: objectId, ref: "User", required: true, immutable: true },
      ownerSnapshot: { type: ownerSnapshotSchema },
      title: textField({ required: true, min: 5, max: 180 }),
      description: textField({ required: true, min: 20, max: 10000 }),
      category: textField({ required: true, max: 80 }),
      skillRequirements: {
        ...
        validate: [
          ...
          "Skill requirements must be unique and at most 20",
        ],
      },
      workMode: {
        type: String,
      ...
      locationText: textField({ max: 160 }),
      visibility: {
        type: String,
        enum: ["PLATFORM", "UNIVERSITY"],
        default: "PLATFORM",
        required: true,
      },
      universityId: { type: objectId, ref: "University" },
      budget: budgetSchema,
      deadlineAt: Date,
      capacity: intField({ defaultValue: 1, min: 1, max: 100 }),
      acceptedCount: intField({ defaultValue: 0 }),
      proposalCount: intField({ defaultValue: 0 }),
      ...
      archivedFromStatus: {
        type: String,
        enum: ["DRAFT", "PUBLISHED", "COMPLETED", "CLOSED", "CANCELLED"],
      },
...
gigSchema.index(
  ...
  },
  {
    ...
    name: "ix_gigs_published_visible_feed",
  },
);
gigSchema.index(
  { ownerId: 1, status: 1, createdAt: -1, _id: -1 },
  { name: "ix_gigs_owner_status_cursor" },
);
gigSchema.index(
  { status: 1, category: 1, createdAt: -1, _id: -1 },
  { name: "ix_gigs_category_feed" },
);
gigSchema.index(
  { status: 1, "skillRequirements.skillId": 1, createdAt: -1, _id: -1 },
  { name: "ix_gigs_skill_feed" },
);
gigSchema.index(
  { status: 1, workMode: 1, createdAt: -1, _id: -1 },
  { name: "ix_gigs_work_mode_feed" },
);
gigSchema.index(
  { status: 1, deadlineAt: 1 },
  { name: "ix_gigs_status_deadline" },
);
gigSchema.index(
  { universityId: 1, status: 1, createdAt: -1, _id: -1 },
  { name: "ix_gigs_university_feed" },
);
gigSchema.index(
  { title: "text", description: "text", category: "text" },
  {
    weights: { title: 10, category: 5, description: 1 },
    name: "tx_gigs_discovery",
  },
);
