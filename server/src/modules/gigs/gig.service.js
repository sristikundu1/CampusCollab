...
export function createGigService({
  config,
  GigModel = Gig,
  ...
  async function affiliation(userId) {
    return AffiliationModel.findOne({ userId, isActive: true }).lean();
  }
  async function activeSkills(ids) {
    ...
    if (skills.length !== new Set(ids.map(String)).size)
      throw new RequestValidationError([
        {
          location: "body",
          path: "skillRequirements",
          code: "inactive_or_unknown_skill",
          message: "Every skill must reference an active catalogue entry.",
        },
      ]);
    return skills;
  }
  async function ownerContext(userId) {
    const [profile, ownerAffiliation] = await Promise.all([
      ProfileModel.findOne({ userId, moderationStatus: "VISIBLE" }).lean(),
      affiliation(userId),
    ]);
    if (!profile)
      throw new ConflictError(
        "PROFILE_REQUIRED",
        "Create your profile before posting a gig.",
      );
    return { profile, affiliation: ownerAffiliation };
  }
  function visibleFilter(viewerAffiliation) {
    const visibility = viewerAffiliation
      ? {
          $or: [
            { visibility: "PLATFORM" },
            {
              visibility: "UNIVERSITY",
              universityId: viewerAffiliation.universityId,
            },
          ],
        }
      : { visibility: "PLATFORM" };
  ...
  async function enrich(gigs, viewerId, ownerView = false) {
    ...
    const skillIds = [
      ...new Set(
        gigs.flatMap((gig) =>
          gig.skillRequirements.map((entry) => String(entry.skillId)),
        ),
      ),
    ];
    const universityIds = [
      ...new Set(
        gigs
          .map((gig) => gig.universityId && String(gig.universityId))
          .filter(Boolean),
      ),
    ];
    ...
    return gigs.map((gig) => ({
      ...
      skills: gig.skillRequirements.map((entry) => ({
        id: String(entry.skillId),
        name: skillMap.get(String(entry.skillId))?.name ?? "Unavailable skill",
        level: entry.level,
      ...
      university:
        gig.universityId && universityMap.get(String(gig.universityId))
          ? {
              id: String(gig.universityId),
              name: universityMap.get(String(gig.universityId)).name,
            }
          : null,
      budget: gig.budget ?? null,
      deadlineAt: gig.deadlineAt ?? null,
      ...
      publishedAt: gig.publishedAt ?? null,
      createdAt: gig.createdAt,
      updatedAt: gig.updatedAt,
      ...(ownerView
        ? { version: gig.version, materialRevision: gig.materialRevision }
        : {}),
    }));
  }
  function cursorFilter(
    ...
    return {
      $or: [
        { [field]: { [op]: new Date(decoded.at) } },
        { [field]: new Date(decoded.at), _id: { [op]: decoded.id } },
      ],
    };
  }
  ...
  async function create(userId, input) {
    ...
    await activeSkills(input.skillRequirements.map((entry) => entry.skillId));
    if (input.visibility === "UNIVERSITY" && !context.affiliation)
      throw new ConflictError(
        "UNIVERSITY_AFFILIATION_REQUIRED",
        "A university affiliation is required for university-only gigs.",
      );
    const gig = await GigModel.create({
      ...
      universityId:
        input.visibility === "UNIVERSITY"
          ? context.affiliation.universityId
          : undefined,
      ...input,
  ...
  async function list(input, viewerId) {
    ...
    const field = input.sort === "DEADLINE" ? "deadlineAt" : "createdAt";
    const direction = input.sort === "DEADLINE" ? "asc" : "desc";
    const filter = {
      $and: [
        visibleFilter(viewerAffiliation),
        cursorFilter(input.cursor, scope, field, direction),
      ],
    };
    if (input.q) filter.$text = { $search: input.q };
    if (input.skillId) filter["skillRequirements.skillId"] = input.skillId;
    if (input.category)
      filter.category = { $regex: escapeRegex(input.category), $options: "i" };
    ...
    const found = await GigModel.find(filter)
      ...
      .limit(input.limit + 1)
      .lean();
    const result = page(found, input.limit, scope, field);
    return {
      gigs: await enrich(result.values, viewerId),
  ...
  async function mine(userId, input) {
    ...
    return {
      gigs: await enrich(result.values, userId, true),
      nextCursor: result.nextCursor,
      hasMore: result.hasMore,
    };
  }
  async function get(gigId, viewerId) {
    ...
    if (!isOwner) {
      ...
      const visible =
        ...
        gig.isActive !== false &&
        gig.moderationStatus === "VISIBLE" &&
        (gig.visibility === "PLATFORM" ||
          (viewerAffiliation &&
            String(viewerAffiliation.universityId) ===
  ...
  async function update(userId, gigId, input) {
    ...
    if (!["DRAFT", "PUBLISHED"].includes(gig.status))
      throw new ConflictError(
        "INVALID_STATE",
        "This gig can no longer be edited.",
      );
    if (input.skillRequirements)
      await activeSkills(input.skillRequirements.map((entry) => entry.skillId));
    const context = await ownerContext(userId);
    if (input.visibility === "UNIVERSITY" && !context.affiliation)
      throw new ConflictError(
        "UNIVERSITY_AFFILIATION_REQUIRED",
        "A university affiliation is required.",
      );
    if (input.capacity !== undefined && input.capacity < gig.acceptedCount)
      throw new ConflictError(
        "CAPACITY_CONFLICT",
        "Capacity cannot be lower than accepted participants.",
      );
    for (const field of editFields)
      if (field in input)
        gig[field] =
          field === "deadlineAt" ? dateValue(input[field]) : input[field];
  ...
  async function transition(userId, gigId, action, input = {}) {
    await transaction(async (session) => {
      ...
      if (!gig) throw new NotFoundError();
      const now = new Date();
      const restorablePublished =
        Boolean(gig.publishedAt) && (!gig.deadlineAt || gig.deadlineAt > now);
      const restoredState =
        gig.archivedFromStatus === "DRAFT"
          ? "DRAFT"
          : restorablePublished
            ? "PUBLISHED"
            : "DRAFT";
      ...
      if (action === "publish") {
        if (gig.deadlineAt && gig.deadlineAt <= now)
          throw new ConflictError(
            "INCOMPLETE_RESOURCE",
            "The deadline must be in the future.",
          );
        gig.acceptingProposals = true;
        gig.publishedAt = now;
      } else if (action === "close") {
        gig.acceptingProposals = false;
        gig.closedAt = now;
        gig.statusReasonCode = input.reasonCode ?? "OWNER_CLOSED";
        ...
        gig.acceptingProposals = false;
        gig.cancelledAt = now;
        gig.statusReasonCode = input.reasonCode;
      } else if (action === "archive") {
        gig.archivedFromStatus = gig.status;
        gig.acceptingProposals = false;
        gig.isActive = false;
        gig.archivedAt = now;
      } else if (action === "restore") {
        gig.acceptingProposals = target === "PUBLISHED";
        gig.isActive = true;
        gig.archivedAt = undefined;
        gig.archivedFromStatus = undefined;
        if (target === "PUBLISHED") gig.closedAt = undefined;
      } else if (action === "start") gig.startedAt = now;
      gig.status = target;
      gig.version += 1;
      await gig.save({ session });
      if (["close", "cancel", "archive"].includes(action))
        await ProposalModel.updateMany(
          { gigId: gig._id, status: { $in: ["SUBMITTED", "SHORTLISTED"] } },
          {
            $set: {
              status: "CLOSED",
              closedAt: now,
              decisionReasonCode: `GIG_${action.toUpperCase()}`,
            },
            $inc: { version: 1 },
          },
          { session },
    ...
    return get(gigId, userId);
  }
  async function addBookmark(userId, gigId) {
    const gig = await get(gigId, userId);
    if (gig.status !== "PUBLISHED") throw new NotFoundError();
    try {
      const bookmark = await BookmarkModel.create({ userId, gigId });
      return {
        created: true,
        bookmark: {
          gigId: String(bookmark.gigId),
          createdAt: bookmark.createdAt,
        },
      };
    } catch (error) {
      if (error?.code !== 11000) throw error;
      const bookmark = await BookmarkModel.findOne({ userId, gigId }).lean();
      return {
        created: false,
        bookmark: {
          gigId: String(bookmark.gigId),
  ...
  async function removeBookmark(userId, gigId) {
    await BookmarkModel.deleteOne({ userId, gigId });
  }
  async function remove(userId, gigId) {
    const gig = await GigModel.findOne({ _id: gigId, ownerId: userId });
    if (!gig) throw new NotFoundError();
    if (!["DRAFT", "ARCHIVED"].includes(gig.status))
      throw new ConflictError(
        "PERMANENT_DELETE_NOT_ALLOWED",
        "Close and archive this gig before deleting it permanently.",
      );
    if (gig.proposalCount > 0 || gig.acceptedCount > 0)
      throw new ConflictError(
        "GIG_HAS_COLLABORATION_HISTORY",
        "A gig with proposals or accepted collaborators cannot be permanently deleted.",
      );
    const deleted = await GigModel.deleteOne({
      _id: gig._id,
      ownerId: userId,
      status: gig.status,
      version: gig.version,
    ...
    if (deleted.deletedCount !== 1)
      throw new ConflictError(
        ...
        "The gig changed while it was being deleted. Refresh and try again.",
      );
    await BookmarkModel.deleteMany({ gigId: gig._id });
  }
  async function bookmarks(userId, input) {
    const scope = `bookmarks:${userId}`;
    const filter = { userId, ...cursorFilter(input.cursor, scope) };
    const found = await BookmarkModel.find(filter)
      .sort({ createdAt: -1, _id: -1 })
      .limit(input.limit + 1)
      .lean();
    const result = page(found, input.limit, scope);
    ...
    const gigs = await GigModel.find({
      _id: { $in: result.values.map((entry) => entry.gigId) },
      ...visibleFilter(viewerAffiliation),
    }).lean();
    const order = new Map(
      result.values.map((entry, index) => [String(entry.gigId), index]),
    );
    gigs.sort((a, b) => order.get(String(a._id)) - order.get(String(b._id)));
    return {
      gigs: await enrich(gigs, userId),
      nextCursor: result.nextCursor,
      hasMore: result.hasMore,
    };
  }
