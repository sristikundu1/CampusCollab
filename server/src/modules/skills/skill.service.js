...
export function createSkillService({ SkillModel = Skill } = {}) {
  const serialize = (skill) => ({ id: String(skill._id), name: skill.name, category: skill.category });
  return {
    ...
    async create(userId, input) {
      ...
      const existing = await SkillModel.findOne({ normalizedName, status: 'ACTIVE' }).lean();
      if (existing) return { skill: serialize(existing), created: false };
      try {
        const skill = await SkillModel.create({ name, normalizedName, category: input.category, createdByUserId: userId, updatedByUserId: userId });
        return { skill: serialize(skill), created: true };
      } catch (error) {
        if (error?.code !== 11000) throw error;
        const duplicate = await SkillModel.findOne({ normalizedName, status: 'ACTIVE' }).lean();
