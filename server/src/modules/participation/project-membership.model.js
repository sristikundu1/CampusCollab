...
const roleSnapshotSchema = new Schema(
  {
    roleName: textField({ required: true, max: 100 }),
    skillIds: [{ type: objectId, ref: "Skill" }],
  },
  { _id: false, strict: "throw" },
);
const projectMembershipSchema = addVersion(
  new Schema(
    {
      projectId: {
        type: objectId,
      ...
      sourceId: { type: objectId, required: true, immutable: true },
      status: {
        type: String,
        enum: ["ACTIVE", "LEFT", "REMOVED", "COMPLETED"],
        default: "ACTIVE",
        required: true,
      },
      ...
      removedAt: Date,
      completedAt: Date,
      changedByUserId: { type: objectId, ref: "User" },
...
projectMembershipSchema.index(
  { projectId: 1, userId: 1 },
  {
    ...
    name: "uq_memberships_active_project_user",
  },
);
projectMembershipSchema.index(
  { sourceType: 1, sourceId: 1 },
  { unique: true, name: "uq_memberships_source" },
);
projectMembershipSchema.index(
  { projectId: 1, status: 1, joinedAt: 1 },
...
export const ProjectMembership = model(
  "ProjectMembership",
