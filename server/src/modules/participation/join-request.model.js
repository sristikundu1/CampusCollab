...
const joinRequestSchema = addVersion(new Schema({
  ...
  decisionReasonCode: textField({ max: 80, select: false }), reason: textField({ max: 80, select: false }), submittedAt: { type: Date, required: true, immutable: true }, acceptedAt: Date, rejectedAt: Date, withdrawnAt: Date, expiredAt: Date,
