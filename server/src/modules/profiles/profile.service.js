...
const toDate = (value) =>
  value ? new Date(value) : value === null ? undefined : undefined;
...
export function createProfileService({
  ...
  async function refreshCompletion(profile) {
    const publishedPortfolioCount = await PortfolioModel.countDocuments({
      userId: profile.userId,
      status: "PUBLISHED",
    });
    Object.assign(
      profile,
      calculateProfileCompletion(profile, publishedPortfolioCount),
    );
  ...
  function profileProjection(profile, context, isOwner) {
    const skillById = new Map(
      context.skills.map((skill) => [String(skill._id), skill]),
    );
    const result = {
      ...
      skills: profile.skillEntries.map((entry) => ({
        id: String(entry.skillId),
        name: skillById.get(String(entry.skillId))?.name ?? "Unavailable skill",
        category: skillById.get(String(entry.skillId))?.category ?? "",
        level: entry.level,
      ...
      university: context.university
        ? {
            id: String(context.university._id),
            name: context.university.name,
            shortName: context.university.shortName,
          }
        : null,
      universityVerification: context.affiliation
        ? { status: context.affiliation.status }
        : null,
  ...
  async function publicProfile(userId, viewerId) {
    ...
    if (profile.visibility === "UNIVERSITY" && !isOwner) {
      ...
      if (
        !ownerAffiliation ||
        !viewerAffiliation ||
        String(ownerAffiliation.universityId) !==
    ...
    return profileProjection(
      profile,
      await contextFor(profile),
      Boolean(isOwner),
    );
  }

  async function create(userId, input) {
    if (await ProfileModel.exists({ userId }))
  ...
  async function update(userId, input) {
    ...
    if ("graduationYear" in input)
      profile.graduationYear = input.graduationYear ?? undefined;
    if ("experienceLevel" in input)
      profile.experienceLevel = input.experienceLevel ?? undefined;
    profile.version += 1;
    await refreshCompletion(profile);
    return own(userId);
  }
  ...
  async function updatePortfolio(userId, itemId, input) {
    ...
    for (const field of [
      ...
      if (field in input) item[field] = input[field];
  ...
  async function publicPortfolio(userId, viewerId) {
    ...
    const items = await PortfolioModel.find({ userId, status: "PUBLISHED" })
