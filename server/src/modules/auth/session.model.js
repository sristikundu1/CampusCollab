  ...
  new Schema(
    {
      ...
      familyId: textField({ required: true, max: 128, immutable: true, select: false }),
      status: { type: String, enum: ['ACTIVE', 'ROTATED', 'REVOKED'], default: 'ACTIVE', required: true },
      authMethod: { type: String, enum: ['PASSWORD'], required: true, immutable: true },
...
sessionSchema.index({ tokenHash: 1 }, { unique: true, name: 'uq_sessions_token_hash' });
sessionSchema.index({ userId: 1, status: 1, createdAt: -1 }, { name: 'ix_sessions_user_status_created' });
sessionSchema.index({ expiresAt: 1 }, { expireAfterSeconds: 0, name: 'ttl_sessions_expires' });
