...
const userSchema = addVersion(
  new Schema(
    {
      email: {
        ...textField({ required: true, max: 320 }),
        lowercase: true,
        immutable: false,
      },
      ...
      capabilities: {
        type: [String],
        default: [],
        validate: [(v) => v.length <= 32, "Capabilities exceed maximum"],
        set: (v) => [...new Set(v)],
      },
      adminGrants: {
        type: [adminGrantSchema],
        default: [],
        select: false,
        validate: [(v) => v.length <= 32, "Admin grants exceed maximum"],
      },
