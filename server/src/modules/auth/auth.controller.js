  ...
  return {
    register: async (request, response) =>
      respond(
        response,
        request,
        await authService.register(request.validated.body),
        201,
      ),
    resend: async (request, response) =>
      respond(
        response,
        request,
        await authService.resendVerification(request.validated.body.email),
        202,
      ),
    ...
    login: async (request, response) => {
      ...
      response.cookie(config.sessionCookieName, result.rawToken, {
        ...cookieOptions,
        expires: result.expiresAt,
      });
      respond(response, request, {
        user: result.user,
        csrfToken: hashOpaqueToken(result.rawToken, config.csrfSecret),
      });
    },
    me: async (request, response) =>
      respond(response, request, {
        user: await authService.currentUser(request.auth.user._id),
        csrfToken: hashOpaqueToken(request.rawSessionToken, config.csrfSecret),
      }),
    ...
    forgot: async (request, response) =>
      respond(
        response,
        request,
        await authService.forgotPassword(request.validated.body.email),
        202,
      ),
