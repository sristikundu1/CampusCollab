...
function serviceFor(gig) {
  ...
  const GigModel = {
    ...
    deleteOne: async (filter) => {
      calls.push(["gig", filter]);
      return { deletedCount: 1 };
    },
  };
  const BookmarkModel = {
    deleteMany: async (filter) => {
      calls.push(["bookmarks", filter]);
    },
  };
  return {
    service: createGigService({ config, GigModel, BookmarkModel }),
    calls,
  };
}
...
test("permanent deletion rejects active lifecycle states and collaboration history", async () => {
  ...
  await assert.rejects(
    () => active.remove(USER, GIG),
    (error) =>
      error instanceof ConflictError &&
      error.code === "PERMANENT_DELETE_NOT_ALLOWED",
  );
  const history = serviceFor({
    _id: GIG,
    status: "ARCHIVED",
    proposalCount: 1,
    acceptedCount: 0,
  ...
  await assert.rejects(
    () => history.remove(USER, GIG),
    (error) =>
      error instanceof ConflictError &&
      error.code === "GIG_HAS_COLLABORATION_HISTORY",
  );
});
