...
test("all Phase 3 MVP collections have a registered model", () => {
  ...
  const collections = Object.values(models)
    .map((entry) => entry.collection.collectionName)
    .sort();
  assert.deepEqual(
    collections,
    [
...
test("sensitive credential fields are excluded by default", () => {
  ...
  assert.equal(
    VerificationChallenge.schema.path("tokenHash").options.select,
    false,
  );
});
...
test("gigs have a backend-controlled active flag for reversible archiving", async () => {
  const gig = new Gig({
    ...
    title: "Build a campus website",
    description:
      "Create a professional responsive website for a student organization.",
...
test("TTL and critical unique indexes match Phase 3", () => {
  const sessionTtl = Session.schema
    .indexes()
    .find(([keys]) => keys.expiresAt === 1);
  const challengeTtl = VerificationChallenge.schema
    .indexes()
    .find(([keys]) => keys.expiresAt === 1);
  assert.equal(sessionTtl[1].expireAfterSeconds, 0);
  assert.equal(challengeTtl[1].expireAfterSeconds, 0);
  const proposalUnique = Proposal.schema
    .indexes()
    .find(
      ([, options]) => options.name === "uq_proposals_active_gig_applicant",
    );
  assert.equal(proposalUnique[1].unique, true);
  assert.deepEqual(proposalUnique[1].partialFilterExpression.status.$in, [
    "SUBMITTED",
    "SHORTLISTED",
    "ACCEPTED",
  ]);
...
test("every explicit index name is globally unique", () => {
  const names = Object.values(models).flatMap((entry) =>
    entry.schema
      .indexes()
