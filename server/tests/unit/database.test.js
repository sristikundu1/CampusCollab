...
function fakeAdapter(connectImplementation) {
  ...
  return {
    connection,
    async connect(uri, options) { await connectImplementation(uri, options, connection); },
...
test('database manager connects once and becomes ready', async () => {
  ...
  const config = { mongodbUri: 'mongodb://placeholder/test', mongodbDbName: 'CampusCollab', isProduction: false };
...
test('database manager exposes failure without leaking URI', async () => {
  const adapter = fakeAdapter(async () => { throw new Error('connection refused'); });
  const manager = createDatabaseManager(adapter);
  await assert.rejects(manager.connect({ mongodbUri: 'mongodb://secret/test', isProduction: false }, logger));
