...
const authService = {
  async authenticate(token) {
    ...
    return { user: { _id: USER }, session: { _id: SKILL } };
  },
};
...
async function run(work) {
  ...
  const skillService = {
    ...
    async create(userId, body) {
      calls.push([String(userId), body]);
      return { created: true, skill: { id: SKILL, ...body } };
    },
  };
  const app = createApp({
    config,
...
test("authenticated students can create a custom skill from session identity", () =>
    ...
    assert.equal(
      (
        await fetch(`${base}/api/v1/skills`, {
          ...
          body: JSON.stringify({ name: "Three.js", category: "Frontend" }),
        })
      ).status,
      401,
    );
...
test("custom skills reject identity and status spoofing", () =>
  run(async (base, calls) => {
    for (const extra of [
      { createdByUserId: "bbbbbbbbbbbbbbbbbbbbbbbb" },
      { status: "ACTIVE" },
