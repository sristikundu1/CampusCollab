...
const valid = {
  NODE_ENV: "test",
  PORT: "5050",
  MONGODB_URI: "mongodb://127.0.0.1:27017/campuscollab_test",
...
test("configuration accepts explicit MongoDB DNS resolver IPs", () => {
  ...
  assert.equal(
    safeConfigurationSummary(config).mongodbDnsOverrideConfigured,
    true,
  );
  assert.throws(
    () =>
      parseEnvironment({
        ...valid,
        MONGODB_DNS_SERVERS: "resolver.example.com",
      }),
    ConfigurationError,
  );
});
...
test("configuration rejects a missing or placeholder MONGODB_URI", () => {
  assert.throws(
    () => parseEnvironment({ ...valid, MONGODB_URI: "" }),
    ConfigurationError,
  );
  assert.throws(
    () =>
