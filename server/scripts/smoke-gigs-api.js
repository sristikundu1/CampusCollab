...
try{
  ...
  result=await request(`/gigs/${gigId}`);if(result.response.status!==200||'version'in result.body.data.gig)throw new Error('Public gig projection failed');
  ...
  await request(`/gigs/${gigId}/bookmark`,mutation(viewer,'DELETE'));await request(`/gigs/${gigId}/bookmark`,mutation(viewer,'DELETE'));
  process.stdout.write('Live gigs smoke test passed: create, ownership, spoof rejection, publish, search, filters, cursor pagination, detail privacy, bookmarks, edit, lifecycle, cleanup.\n');
}finally{
