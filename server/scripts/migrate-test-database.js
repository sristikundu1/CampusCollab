...
if (config.isProduction) {
  process.stderr.write(
    "Development database migration is disabled in production.\n",
  );
  process.exit(1);
}
...
try {
  ...
  const sourceCollections = new Set(
    (await source.listCollections({}, { nameOnly: true }).toArray()).map(
      (entry) => entry.name,
    ),
  );
  const modelByCollection = new Map(
    Object.values(models).map((entry) => [
  ...
  process.stdout.write(
    `Migrated CampusCollab data from ${SOURCE_DATABASE} to ${config.mongodbDbName}: ${discovered} found, ${inserted} newly inserted. Source data was preserved.\n`,
  );
} catch (error) {
  process.stderr.write(
    `Database migration failed (${error?.name ?? "Error"}).\n`,
  );
  process.exitCode = 1;
} finally {
