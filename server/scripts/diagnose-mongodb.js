...
const shape = {
  present: Boolean(uri),
  scheme: uri.startsWith('mongodb+srv://') ? 'mongodb+srv' : uri.startsWith('mongodb://') ? 'mongodb' : 'invalid',
...
try {
  ...
  process.stdout.write(`${JSON.stringify({ connected: true, database: mongoose.connection.name, shape })}\n`);
