...
export function BookmarksPage() {
  ...
  const load = useCallback(async (cursor, append = false) => {
    ...
    try {
      const response = await gigApi.bookmarks(cursor ? { cursor } : {});
      setGigs((current) =>
        append
    ...
    <AppShell>
      <div>
        ...
        <p className="mt-2 text-slate-600">
          Keep promising opportunities within reach.
        </p>
        {loading ? (
          <div className="mt-7 h-80 animate-pulse rounded-2xl bg-slate-200" />
        ) : error ? (
          ...
          <div className="surface mt-7 p-10 text-center">
            ...
            <h2 className="mt-4 text-xl font-bold">No bookmarked gigs</h2>
            <p className="mt-2 text-slate-600">
              Save opportunities from the marketplace to find them here.
            </p>
