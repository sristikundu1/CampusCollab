...
export function ProjectDetailsPage() {
  ...
  useEffect(() => {
    void projectApi
      ...
      .catch((e) =>
        setError(
          apiError(e).status === 404
            ? "This project is not visible or no longer available."
            : apiError(e).message,
        ),
      );
  }, [projectId]);
  const join = async () => {
    if (!isAuthenticated) {
      notify("Sign in before requesting to join.", "error");
      navigate("/login", { state: { from: `/projects/${projectId}` } });
      return;
    }
    if (!selected || message.trim().length < 1) {
      notify("Choose a role and add a short message.", "error");
      return;
    }
    setBusy(true);
    try {
      await participationApi.requestJoin(projectId, selected, message.trim());
    ...
    return (
      <MarketplaceLayout>
        <div className="surface mx-auto max-w-xl p-10 text-center">
          <h1 className="text-xl font-black">Project unavailable</h1>
  ...
  return (
    <MarketplaceLayout>
      <div className="mx-auto max-w-6xl">
        ...
        <section className="mt-5 overflow-hidden rounded-3xl bg-[radial-gradient(circle_at_82%_15%,rgba(34,211,238,.32),transparent_28%),linear-gradient(135deg,#1e1b4b,#1d4ed8)] p-7 text-white sm:p-10">
          ...
          <h1 className="mt-5 max-w-4xl text-3xl font-black sm:text-4xl">
            {project.title}
          </h1>
          <p className="mt-5 flex items-center gap-2 text-sm font-bold text-blue-100">
            <ShieldCheck size={17} />
            {project.owner.displayName} · University community member
          </p>
        </section>
        <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_350px]">
            ...
            <section>
              ...
              <div className="mt-4 space-y-4">
                {project.openings.map((o) => (
                  <article
                    key={o.id}
                    className={`surface p-6 ${selected === o.id ? "ring-2 ring-indigo-500" : ""}`}
                  >
                    <div className="flex flex-wrap justify-between gap-3">
                      <div>
                        <h3 className="text-lg font-black">{o.roleName}</h3>
                        <p className="mt-2 text-sm leading-6 text-slate-600">
                          {o.description}
                        </p>
                      </div>
                      <span className="h-fit rounded-full bg-slate-100 px-3 py-1 text-xs font-black">
                        {o.remainingCapacity} of {o.capacity} available
                      </span>
                    </div>
          ...
          <aside className="h-fit space-y-5 lg:sticky lg:top-24">
            <section className="surface p-6">
              ...
              <dl className="mt-5 space-y-4 text-sm">
                <div className="flex gap-3">
                  ...
                  <div>
                    <dt className="text-slate-500">Recruitment</dt>
                ...
                <div className="flex gap-3">
                  ...
                  <div>
                    <dt className="text-slate-500">Expected timeline</dt>
                    <dd className="font-bold">
                      {project.expectedStartAt
                        ? new Date(project.expectedStartAt).toLocaleDateString()
                        : "Flexible"}{" "}
                      —{" "}
                      {project.expectedEndAt
                        ? new Date(project.expectedEndAt).toLocaleDateString()
                        : "Open"}
                    </dd>
                  </div>
                </div>
            ...
            {selected && !project.isOwner && (
              <section className="surface p-6">
                <h2 className="text-lg font-black">Join this team</h2>
                <p className="mt-2 text-sm text-slate-500">
                  Introduce yourself and explain how you can contribute.
                </p>
                <textarea
                  ...
                  placeholder="I would like to contribute because…"
                />
                <button
