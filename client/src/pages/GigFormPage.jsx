...
export function GigFormPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([skillApi.list(), ...(editing ? [gigApi.get(gigId)] : [])])
      .then(([skillResponse, gigResponse]) => {
        if (!active) return;
        setSkills(skillResponse.data.data.skills);
        if (editing) {
          if (!gigResponse.data.data.gig.isOwner)
            throw Object.assign(new Error("Forbidden"), {
              response: {
                status: 403,
                data: {
                  error: { message: "Only the owner can edit this gig." },
                },
              },
            });
          setGig(gigResponse.data.data.gig);
        }
      })
  ...
  return (
    <AppShell>
      <div className="mx-auto max-w-4xl">
        ...
        <p className="eyebrow mt-6">Gig owner workspace</p>
        <h1 className="mt-2 text-3xl font-bold">
          {editing ? "Edit gig" : "Create a gig"}
        </h1>
        <p className="mt-2 text-slate-600">
          {editing
            ? "Update only the terms that remain editable in this lifecycle state."
            : "Create a clear draft, review it, then publish it from My Gigs."}
        </p>
        {loading ? (
          <div className="mt-7 h-96 animate-pulse rounded-3xl bg-slate-200" />
        ) : error ? (
