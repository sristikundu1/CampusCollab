  ...
  const submit = async (values) => {
    if (!token.current)
      return setError("root", {
        message: "This reset link is missing its secure token.",
      });
    try {
      await authApi.reset({ token: token.current, ...values });
      setDone(true);
    } catch (e) {
      setError("root", { message: apiError(e).message });
    }
  };
  return (
    <AuthLayout
      ...
      subtitle="Your new password will sign out any other active CampusCollab sessions."
    >
      {done ? (
