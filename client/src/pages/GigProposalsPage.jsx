...
export function GigProposalsPage() {
  const { gigId } = useParams();
  const { notify } = useToast();
  const [data, setData] = useState({ gig: null, proposals: [] });
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const response = await proposalApi.forGig(gigId);
      setData(response.data.data);
      setError("");
    } catch (reason) {
      setError(
        apiError(reason).status === 404
          ? "This gig is not available in your owner workspace."
  ...
  const decide = async (proposal, action) => {
    const messages = {
      accept: {
        ...
        text: "This reserves one collaboration place. The action cannot be reversed.",
        confirmText: "Accept proposal",
        icon: "question",
      },
      reject: {
        title: "Reject this proposal?",
        text: "The applicant will see that they were not selected.",
    ...
    setBusy(proposal.id);
    try {
      const response = await proposalApi[action](
        proposal.id,
        action === "reject" ? { reasonCode: "NOT_SELECTED" } : {},
      );
      setData((current) => ({
        ...current,
        gig: response.data.data.gig,
        proposals: current.proposals.map((item) =>
          item.id === proposal.id ? response.data.data.proposal : item,
        ),
      }));
      notify(
        action === "accept"
    ...
    return (
      <AppShell>
        <div className="surface mx-auto max-w-xl p-8 text-center">
          ...
          <h1 className="mt-3 text-xl font-bold">Owner review unavailable</h1>
        ...
        <div className="mt-5 flex flex-col justify-between gap-4 sm:flex-row sm:items-end">
          <div>
            ...
            <h1 className="mt-2 text-3xl font-black">
              Proposals for {data.gig.title}
            </h1>
            <p className="mt-2 text-slate-600">
              {data.gig.acceptedCount} of {data.gig.capacity} places accepted.
            </p>
          </div>
          <Link className="btn-secondary" to={`/gigs/${gigId}`}>
            View gig
          </Link>
        </div>
        {data.proposals.length === 0 ? (
          ...
          <div className="mt-7 grid gap-5">
            {data.proposals.map((proposal) => (
              <article className="surface p-5 sm:p-6" key={proposal.id}>
                <div className="flex flex-col justify-between gap-5 sm:flex-row sm:items-start">
                  <div className="min-w-0">
                    ...
                    <p className="mt-1 text-sm text-slate-500">
                      ...
                      {proposal.applicant.profile
                        ? ` · ${proposal.applicant.profile.completionScore}% profile`
                        : ""}
                    </p>
                  ...
                  <div className="flex shrink-0 flex-wrap gap-2">
                    ...
                    {proposal.status === "SUBMITTED" && (
                      <button
                        className="inline-flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-4 py-2 text-sm font-bold text-amber-800"
                        disabled={busy === proposal.id}
                        onClick={() => decide(proposal, "shortlist")}
                      >
                        <Star size={15} />
                        Shortlist
                      </button>
                    )}
                    {active.includes(proposal.status) && (
                      <>
                        <button
                        ...
                        <button
                          className="inline-flex items-center gap-2 rounded-xl border border-rose-200 px-4 py-2 text-sm font-bold text-rose-700"
                          disabled={busy === proposal.id}
