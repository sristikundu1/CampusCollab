...
vi.mock("../services/api.js", () => ({
  profileApi: {
    own: mocks.own,
    ownPortfolio: mocks.ownPortfolio,
    update: mocks.update,
    replaceSkills: mocks.replaceSkills,
...
const profile = {
  ...
  department: "CSE",
  graduationYear: 2027,
  bio: "I build useful campus products.",
  experienceLevel: "INTERMEDIATE",
  visibility: "PLATFORM",
  version: 1,
  availability: { status: "AVAILABLE", hoursPerWeek: 12, availableFrom: null },
  skills: [],
  externalLinks: [],
  completionScore: 60,
  isCompleteForApplications: false,
  university: { name: "United International University" },
  universityVerification: { status: "PENDING" },
};
const skills = [
  { id: "dddddddddddddddddddddddd", name: "React", category: "Frontend" },
  { id: "eeeeeeeeeeeeeeeeeeeeeeee", name: "Node.js", category: "Backend" },
];
      ...
      <AuthContext.Provider
        value={{
          user: {
            ...
            profile: { displayName: "Sristi Kundu" },
          },
          refreshUser: vi.fn(),
          logout: vi.fn(),
        }}
...
beforeEach(() => {
  ...
  mocks.createSkill.mockImplementation((body) =>
    response({ skill: { id: "ffffffffffffffffffffffff", ...body } }),
  );
  mocks.update.mockImplementation((body) =>
    response({ profile: { ...profile, ...body, version: 2 } }),
  );
  mocks.replaceSkills.mockImplementation((entries) =>
    response({
      profile: {
        ...profile,
  ...
  mocks.updateAvailability.mockImplementation((availability) =>
    response({ profile: { ...profile, availability, version: 2 } }),
  );
  mocks.createPortfolio.mockImplementation((body) =>
    response({ item: { id: "cccccccccccccccccccccccc", ...body } }),
  );
});
afterEach(() => {
  cleanup();
...
describe("ProfilePage", () => {
  it("loads real profile data and the portfolio empty state", async () => {
    ...
    expect(
      screen.getByText("United International University"),
    ).toBeInTheDocument();
  ...
  it("edits and saves allowlisted profile fields", async () => {
    ...
    await user.type(headline, "MERN product engineer");
    await user.click(screen.getByRole("button", { name: "Save profile" }));
    await waitFor(() => expect(mocks.update).toHaveBeenCalled());
    expect(mocks.update.mock.calls[0][0].headline).toBe(
  ...
  it("adds a canonical skill and updates approved availability", async () => {
    ...
    await waitFor(() =>
      expect(mocks.replaceSkills).toHaveBeenCalledWith([
        { skillId: skills[0].id, level: "BEGINNER", evidence: "" },
      ]),
    );
  ...
  it("creates a portfolio project from the empty state", async () => {
    ...
    await user.click(
      screen.getByRole("button", { name: "Add your first project" }),
    );
    await user.type(screen.getByLabelText("Project title"), "CampusCollab");
    await user.type(
      screen.getByLabelText("Description"),
      "A trusted collaboration platform for university students.",
    );
