...
export function GigCard({ gig, onBookmark, busy = false, ownerActions }) {
  ...
  return <article className="group flex h-full flex-col overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_10px_35px_rgba(15,23,42,.06)] transition duration-300 hover:-translate-y-1 hover:border-brand-200 hover:shadow-[0_20px_50px_rgba(37,86,204,.12)]">
    ...
    <div className="mt-5 flex flex-wrap gap-2"><span className="rounded-full bg-brand-50 px-3 py-1 text-xs font-bold text-brand-700">{gig.category}</span>{gig.isOwner&&<span className={`rounded-full px-3 py-1 text-xs font-bold ${gig.isActive===false?'bg-slate-100 text-slate-600':'bg-emerald-50 text-emerald-700'}`}>{gig.isActive===false?'Inactive':'Active'}</span>}</div>
