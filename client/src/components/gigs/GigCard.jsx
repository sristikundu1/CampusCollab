...
export function GigCard({ gig, onBookmark, busy = false, ownerActions }) {
  return <article className="surface flex h-full flex-col p-5 transition hover:-translate-y-0.5 hover:shadow-lg">
    ...
    <div className="mt-auto border-t border-slate-100 pt-4"><div className="flex items-center justify-between gap-3"><Link to={`/students/${gig.owner.id}`} className="text-sm font-bold text-slate-700 hover:text-brand-700">{gig.owner.displayName}</Link><Link className="text-sm font-bold text-brand-700" to={`/gigs/${gig.id}`}>View details →</Link></div>{ownerActions && <div className="mt-3 flex flex-wrap gap-2 border-t border-slate-100 pt-3" aria-label="Gig management actions">{ownerActions}</div>}</div>
  </article>;
}
