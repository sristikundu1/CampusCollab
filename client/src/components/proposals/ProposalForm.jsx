...
const schema = z
  .object({
    coverMessage: z
      ...
      .max(5000),
    budgetType: z.enum(["MATCH_GIG", "FIXED", "RANGE", "UNPAID"]),
    minAmount: z.string().optional(),
    maxAmount: z.string().optional(),
    currency: z.string().trim().optional(),
    proposedDuration: z.string().trim().max(120).optional(),
    availability: z.string().trim().max(300).optional(),
  })
  .superRefine((value, context) => {
    if (
      ["FIXED", "RANGE"].includes(value.budgetType) &&
    ...
    if (
      ...
      (!value.maxAmount || Number(value.maxAmount) < Number(value.minAmount))
    )
      context.addIssue({
        code: "custom",
        path: ["maxAmount"],
...
function toPayload(values, gigBudget) {
  let proposedBudget;
  if (values.budgetType === "MATCH_GIG") proposedBudget = gigBudget;
  else if (values.budgetType === "UNPAID") proposedBudget = { type: "UNPAID" };
  else
    proposedBudget = {
...
export function ProposalForm({
  ...
  const {
    ...
    defaultValues: {
      ...
      minAmount:
        current?.minMinor !== undefined ? String(current.minMinor / 100) : "",
      maxAmount:
        current?.maxMinor !== undefined ? String(current.maxMinor / 100) : "",
      currency: current?.currency ?? gigBudget?.currency ?? "BDT",
      proposedDuration: initial?.proposedDuration ?? "",
  ...
  return (
    <form
      ...
      onSubmit={handleSubmit((values) =>
        onSubmit(toPayload(values, gigBudget)),
      )}
      noValidate
    >
      <FormField
      ...
      >
        <textarea
          ...
          placeholder="Explain your approach, relevant experience, and why you are a strong fit."
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        >
          <select id="budgetType" className="field" {...register("budgetType")}>
            <option value="MATCH_GIG">Match gig budget</option>
        ...
        <FormField
          ...
          error={errors.proposedDuration?.message}
        >
          <input
            id="proposedDuration"
            className="field"
            placeholder="Example: 2 weeks"
            {...register("proposedDuration")}
          />
        </FormField>
        <FormField
          id="proposalAvailability"
          label="Availability"
        ...
        >
          <input
            id="proposalAvailability"
            className="field"
            placeholder="Example: 10 hours per week"
