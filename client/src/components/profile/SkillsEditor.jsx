...
export function SkillsEditor({ profileSkills, catalogue, saving, onSave, onCreateSkill }) {
  ...
  const createSkill = async (input) => { const skill = await onCreateSkill(input); if (skill) setSelected((current)=>({ ...current, [skill.id]: { skillId: skill.id, level: 'BEGINNER', evidence: '' } })); return skill; };
  return <section className="surface p-6">
    ...
    <div className="mt-3"><CustomSkillForm compact onCreate={createSkill}/></div>
