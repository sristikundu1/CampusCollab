...
export function SkillsEditor({
  ...
  const toggle = (skill) =>
    setSelected((current) =>
      current[skill.id]
        ? Object.fromEntries(
            Object.entries(current).filter(([id]) => id !== skill.id),
          )
        : {
            ...current,
  ...
  const setLevel = (id, level) =>
    setSelected((current) => ({ ...current, [id]: { ...current[id], level } }));
    ...
    <section className="surface p-6">
      ...
      <div className="relative mt-5">
        ...
        <input
          ...
          placeholder="Search the skill catalogue"
      ...
      <div className="mt-4 grid max-h-80 gap-2 overflow-y-auto pr-1 sm:grid-cols-2">
        {visible.map((skill) => {
          ...
          return (
            <div
              key={skill.id}
              className={`rounded-xl border p-3 ${active ? "border-brand-300 bg-brand-50" : "border-slate-200 bg-white"}`}
            >
              <div className="flex items-center justify-between gap-2">
                ...
                >
                  <span
                    className={`grid size-6 shrink-0 place-items-center rounded-md ${active ? "bg-brand-600 text-white" : "border border-slate-300"}`}
                  >
              ...
              {active && (
                <select
                  ...
                  className="mt-3 w-full rounded-lg border border-brand-200 bg-white px-2 py-2 text-xs font-semibold"
                  value={active.level}
                  onChange={(event) => setLevel(skill.id, event.target.value)}
                >
